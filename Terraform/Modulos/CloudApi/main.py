...
# from flask import Flask, request, jsonify
# from google.cloud import pubsub_v1
# from google.cloud.sql.connector import connect
...
# @app.route('/api/resources-assigned', methods=['GET'])
...
#         # Consulta SQL
#         cur.execute("SELECT recurso_id, servicio, latitud, longitud FROM recursos WHERE asignado = TRUE")
#         rows = cur.fetchall()
...
#         if not rows:
#             return jsonify({"message": "No assigned resources found"}), 404

#         # Convertir los resultados a un formato adecuado para la respuesta
#         resources = [{"recurso_id": row[0], "servicio": row[1], "latitud": row[2], "longitud": row[3]} for row in rows]

#         return jsonify({"assigned_resources": resources}), 200

#     except Exception as e:
#         print(f"Error fetching assigned resources: {e}")
#         return jsonify({"error": "Failed to fetch assigned resources"}), 500
