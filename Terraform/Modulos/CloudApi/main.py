...
@app.route('/api/status-evento/<evento_id>', methods=['GET'])
def status_evento(evento_id):
    client = bigquery.Client()

    dataset_id = "emergencia_eventos"
    tabla_macheadas = "emergencias-macheadas"
    tabla_no_macheadas = "emergencias-no-macheadas"

    try:
        # Consulta tabla de eventos macheados
        query_macheado = f"""
        SELECT evento_id
        FROM `{PROJECT_ID}.{dataset_id}.{tabla_macheadas}`
        ...
        result = client.query(query_macheado, job_config=job_config).result()
        if result.total_rows > 0:
            return jsonify({"evento_id": evento_id, "mensaje": "La ayuda llegará en breves momentos"}), 200

        # Consulta tabla de eventos no macheados
        query_no_macheado = f"""
        SELECT evento_id
        FROM `{PROJECT_ID}.{dataset_id}.{tabla_no_macheadas}`
        WHERE evento_id = @evento_id
        LIMIT 1
        """
        job_config.query_parameters[0] = bigquery.ScalarQueryParameter("evento_id", "STRING", evento_id)
        result = client.query(query_no_macheado, job_config=job_config).result()
        if result.total_rows > 0:
            return jsonify({"evento_id": evento_id, "mensaje": "No hay recursos disponibles en este momento"}), 200

        return jsonify({"evento_id": evento_id, "mensaje": "Evento no encontrado"}), 404

    except Exception as e:
        print(f"Error al consultar BigQuery: {e}")
        return jsonify({"error": "Error consultando el estado del evento"}), 500
